...
jQuery(document).bind('cleverswatch_update_gallery', function (event, response) {
    if(jQuery('.custom-product-page')[0]){
        var gallery=jQuery(response.content).find('.woocommerce-product-gallery').html();
        console.log(gallery);
        jQuery('#product-' + response.product_id).find('.images').html(gallery);
        var thumb=jQuery(response.content).find('.product-thumbnails');
        jQuery('#product-' + response.product_id).find('.vertical-thumbnails').html(thumb);
    }else{
